...
/obj/structure/roguemachine/scomm/attack_right(mob/user)
	...
	if(SSticker.rulertype == "Lord")
		...
		contents += "<center>Lady' DECREES<BR>"
...
/obj/structure/roguemachine/scomm/MiddleClick(mob/living/carbon/human/user)
	if(.)
		return
	if((HAS_TRAIT(user, TRAIT_GUARDSMAN) || (HAS_TRAIT(user, TRAIT_KNIGHTSMAN)) || (user.job == "Warden") || (user.job == "Squire") || (user.job == "Marshal") || (user.job == "Lord") || (user.job == "Knight Captain") || (user.job == "Lady")))
